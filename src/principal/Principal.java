...
public class Principal {
	...
	public static void main(String[] args) {
		...
		int[] clasi2 = prac.obtenerClasificacionColumnas(dat.getResultados());
		for (int i = 0; i < clasi2.length; i++) {
			System.out.println(clasi2[i]);
		}

		System.out.println("------------CLASIFICACIÓN------------");
		int[][] ptsJornada = new Datos().getPtsJornada();
		Equipo[] clasificacion = prac.obtenerClasificacionJornadas(ptsJornada);
		for (int i = 0; i < clasificacion.length; i++) {
			System.out.println(clasificacion[i].getNombre() + clasificacion[i].getPuntos());
		}
		...
		int n4 = cade.indexOf("cde");
		System.out.println(n4);
		
		
		boolean primo  = prac.esPrimo(3);
		System.out.println(primo);
		
		
		int[] lista = prac.listaPri(20);
		for (int i = 0; i < lista.length; i++) {
			System.out.println(lista[i]);
		}
		
		int[] lista2 = prac.listaPri2(20);
		...
		System.out.println("------------");
		int[] listFibo = prac.numFibo(10);
		for (int i = 0; i < listFibo.length; i++) {
			System.out.println(listFibo[i]);
		}
	}
}
